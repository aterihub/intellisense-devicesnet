...
export class GatewaysController {
  ...
  async findOne(@Param('id') id: string) {
    const device = await this.gatewaysService.findOne({ id });
    const gatewaysEntity = new GatewaysEntity(device);
    return {
      status: 'success',
      data: { device: gatewaysEntity },
    };
  }
  ...
  async create(@Body() data: CreateGatewayDto) {
    const device = await this.gatewaysService.create(
  ...
  async update(@Param('id') id: string, @Body() data: UpdateGatewayDto) {
    const device = await this.gatewaysService.update({
      where: { id },
